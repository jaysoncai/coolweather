...
public class WeatherActivity extends AppCompatActivity {
    ...
    private void queryWeatherData() {
        ...
        if (weatherJson != null) {
            Log.d(TAG, "queryWeatherData: weatherJson=" + weatherJson);
            // 从缓存中获取
            ...
            Log.d(TAG, "queryWeatherData: weatherId=" + weatherId);
            mScrollView.setVisibility(View.INVISIBLE);  // 请求数据时隐藏滚动条
            queryWeatherFromServer(weatherId);
        }
    }

    private void queryWeatherFromServer(String weatherId) {
